(() => {
  ...
  function shader(type, source) {
    ...
    if (!result) throw new Error("Shader no disponible");
    shaders.push(result);
  ...
  try {
    ...
    if (!program) throw new Error("Programa no disponible");
    ...
    gl.attachShader(program, shader(gl.FRAGMENT_SHADER, `
      ...
      void main() {
        ...
        float t = time * 0,13;
        float x = p.x + (pointer.x - 0,5) * 0,065;
        float y = p.y + (pointer.y - 0,5) * 0,025;
        float fold = sin(x * 6,2 + sin(y * 4,5 - t) * 1,1 + t);
        float sweep = x + y * 0,48 + fold * 0,13;
        float ribbon = 0,5 + 0,5 * sin(sweep * 15. - t * 0,7);
        float fine = pow(0,5 + 0,5 * sin(sweep * 29. + t * 0,3), 12.);
        float light = exp(-pow((sweep - 0,82 - sin(t) * 0,07) * 3,2, 2.));
        vec3 cream = vec3(0,922, 0,871, 0,775);
        vec3 salvia = vec3(0,56, 0,67, 0,52);
        vec3 oro = vec3(0,91, 0,72, 0,44);
        vec3 color = mix(crema, salvia, cinta * 0,40);
        color = mix(color, gold, light * 0,34);
        color += vec3(0,12, 0,105, 0,08) * fine;
        // Mantener sin cambios el lado izquierdo del encabezado y los bordes de la sección.
        float intensidad = smoothstep(0,12, 0,67, p.x) * (1 - smoothstep(0,65, 1, p.y));
        intensidad *= smoothstep(0., 0,10, p.y);
        color = mix(crema, color, intensidad);
        gl_FragColor = vec4(color, 1.);
      }
    `));
    gl.bindAttribLocation(program, 0, 'position');
    gl.linkProgram(program);
    if (!gl.getProgramParameter(program, gl.LINK_STATUS)) throw new Error("Error en el enlace del shader");
    gl.useProgram(program);
    buffer = gl.createBuffer();
    if (!buffer) throw new Error("Búfer no disponible");
  ...
  function update() {
    ...
    control.textContent = paused ? "Reproducir efecto de luz" : "Efecto de luz de pausa";
